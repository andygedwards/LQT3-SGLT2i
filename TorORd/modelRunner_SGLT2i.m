...
ODE = 1; if (isfield(parameters,'ODE')) ODE = parameters.ODE;end
if ~ODE == 1 
    if ~isfield(parameters,'times') || ~isfield(parameters,'states_solution')|| ~isfield(parameters,'cellType') || ~isfield(parameters,'extraParams') || ~isfield(parameters,'model')
        error('To run the model to return currents rather than the state vector you must provide inputs for: (1) parameters.times, (2) parameters.states_solution, (3) parameters.cellType, (4) parameters.extraParams, (5) parameters.model')
    elseif isempty(parameters.times) || isempty(parameters.states_solution)|| isempty(parameters.cellType) || isempty(parameters.model)
        error('To run the model to return currents rather than the state vector you must provide inputs for: (1) parameters.times, (2) parameters.states_solution, (3) parameters.cellType, (4) parameters.model')
    end
end
...
if ODE == 1
    ...
    for n=1:beats
        ...
        [time{n}, X{n}]=ode15s(parameters.model,[0 CL],X0,options,ODE,cellType, ICaL_Multiplier, ...
        ...
        if ~isequal(time{n}(end), CL) % If simulation was killed prematurely in ode15sTimed, it is handled separately, setting dummy values.
            toc(timerVar);
    ...
    T = parameters.times;
    states_solution = parameters.states_solution;
    if iscell(T)
        for i = 1:length(T)
            for j = 1:length(T{i})
                X{i}(j,:) = parameters.model(T{i}(j),states_solution{i}(j,:),ODE,cellType, ICaL_Multiplier, ...
                    INa_Multiplier, Ito_Multiplier, INaL_Multiplier, IKr_Multiplier, IKs_Multiplier, IK1_Multiplier, IKb_Multiplier,INaCa_Multiplier,...
                    INaK_Multiplier, INab_Multiplier, ICab_Multiplier, IpCa_Multiplier, ICaCl_Multiplier, IClb_Multiplier, Jrel_Multiplier,Jup_Multiplier,nao,cao,ko,ICaL_fractionSS,INaCa_fractionSS, stimAmp, stimDur,stimStart, vcParameters, apClamp, extraParams);
            end
        end
        ...
        for j = 1:length(T)
            X(j,:) = parameters.model(T(j),states_solution(j,:),ODE,cellType, ICaL_Multiplier, ...
                    INa_Multiplier, Ito_Multiplier, INaL_Multiplier, IKr_Multiplier, IKs_Multiplier, IK1_Multiplier, IKb_Multiplier,INaCa_Multiplier,...
                    INaK_Multiplier, INab_Multiplier, ICab_Multiplier, IpCa_Multiplier, ICaCl_Multiplier, IClb_Multiplier, Jrel_Multiplier,Jup_Multiplier,nao,cao,ko,ICaL_fractionSS,INaCa_fractionSS, stimAmp, stimDur,stimStart, vcParameters, apClamp, extraParams);
        end
    end    
