...
function currents = getCurrentsStructure_SGLT2i(time, X, parameters, ignoreFirstSpikes)
...
stimStart = 50; if (isfield(parameters,'stimStart')) stimStart = parameters.stimStart; end

% Which model is used by default
if(~isfield(parameters,'model'))
    parameters.model = @TorORd_rhs_SGLT2i;
end

% Default CL is 1000 unless otherwise specified
CL = 1000; if (isfield(parameters,'bcl')) CL = parameters.bcl; end
...
for iBeat = 1:size(time, 1)
    ...
    currents.time = [currents.time; (iBeat-1)*CL + time{iBeat}];
    Xhere = X{iBeat};
    ...
    INaK_Multiplier, INab_Multiplier, ICab_Multiplier, IpCa_Multiplier, ICaCl_Multiplier, IClb_Multiplier, Jrel_Multiplier,Jup_Multiplier, nao,cao,ko,ICaL_fractionSS,INaCa_fractionSS, stimAmp, stimDur,stimStart, vcParameters, apClamp, extraParams);
        ...
        currents.A_INaf(i)=IsJs(1);
        currents.A_INaL(i)=IsJs(2);
        currents.B_INaf(i)=IsJs(3);
        currents.B_INaL(i)=IsJs(4);
        currents.Ito(i)=IsJs(5);
        currents.ICaL(i)=IsJs(6);
        currents.IKr(i)=IsJs(7);
        currents.IKs(i)=IsJs(8);
        currents.IK1(i)=IsJs(9);
        currents.INaCa_i(i)=IsJs(10);
        currents.INaCa_ss(i)=IsJs(11);
        currents.INaK(i)=IsJs(12);
        currents.IKb(i)=IsJs(13);
        currents.INab(i)=IsJs(14);
        currents.ICab(i)=IsJs(15);
        currents.IpCa(i)=IsJs(16);
        currents.Jdiff(i)=IsJs(17);
        currents.JdiffNa(i)=IsJs(18);
        currents.JdiffK(i)=IsJs(19);
        currents.Jup(i)=IsJs(20);
        currents.Jleak(i)=IsJs(21);
        currents.Jtr(i)=IsJs(22); 
        currents.Jrel(i)=IsJs(23);
        currents.CaMKa(i)=IsJs(24);
        currents.Istim(i)=IsJs(25);
        currents.Ivclamp(i)=IsJs(26);
        currents.fINap(i) = IsJs(27);
        currents.fINaLp(i) = IsJs(28);
        currents.fICaLp(i) = IsJs(29);
        currents.fJrelp(i) = IsJs(30);
        currents.fJupp(i) = IsJs(31);       
        currents.CaJSR(i) = IsJs(32);
        currents.CaNSR(i) = IsJs(33);
        currents.PhiCaL_ss(i) = IsJs(34);     
        currents.V(i) = IsJs(35); %  This may be usually obtained directly from vector of state variables, but not necessarily when clamping is used, so it can be also extracted separately.
        currents.ICaL_i(i) = IsJs(36);
        currents.ICaCl(i) = IsJs(37);
        currents.IClbk(i) = IsJs(38);
        currents.ICaL_tot(i) = IsJs(39);
